...
namespace {
  ...
  void Erase(T& c, const U& value) {
    // remove collects the target items, but not delete
...
// Add a message to message queue and wake up
...
TaskManager::TaskManager() {
  // put initial task because switchtask is called
  Task& task = NewTask()
...
// @brief Remove the task from the run queue
...
// @brief Remove the task by id from the run queue
...
/** @brief
 * 1. Wake up a task sleeping : add the task to the run queue
 * 2. Change level of the running task
**/
...
// @brief Add the task by id to the run queue
...
// SendMessage() send a message to the indicate task
...
void TaskManager::ChangeLevelRunning(Task* task, int level) {
  ...
  // When task == running_[current_level_].front()
  // change level my self
