...
void Terminal::DrawCursor(bool visible) {
  const auto color = visible ? ToColor(0xa47fff) : ToColor(0xC8B2FF);
...
Rectangle<int> Terminal::InputKey(
  ...
  if (ascii == '\n') {
    ...
    if (cursor_.x > 0) {
      --cursor_.x;
      FillRectangle(*window_->Writer(), CalcCursorPos(), {8, 16}, {200, 178, 255});
...
void Terminal::Scroll1() {
  ...
  FillRectangle(*window_->InnerWriter(),
                {4, 4 + 16*cursor_.y}, {8*kColumns, 16}, {200, 178, 255});
}
...
void Terminal::ExecuteLine() {
  ...
  if (strcmp(command, "echo") == 0) {
    ...
    for (int i = 0; i < entries_per_cluster; ++i) {
      ...
      if (base[0] == 0x00) { // This dictionary entry is vacant, there is no directory entry later
        break;
      } else if (static_cast<uint8_t>(base[0]) == 0xe5) { // This directory entry is vacant, no file and no dir is described
        continue;
...
Rectangle<int> Terminal::HistoryUpDown(int direction) {
  ...
  FillRectangle(*window_->Writer(), draw_area.pos, draw_area.size, {200, 178, 255});
...
void TaskTerminal(uint64_t task_id, int64_t data) {
  ...
  // Register Layer ID and Task Map
